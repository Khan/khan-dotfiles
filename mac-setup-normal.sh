...
install_node() {
    ...
    # The following instructions were generated by https://nodejs.org/en/download.
    # Download and install nvm:
    # If nvm is already installed, this will update it to the latest version.
    ...
    if ! node --version | grep "v20" >/dev/null ; then
        ...
        if type nvm >/dev/null 2>&1; then
            notice "To install and use node v20 with nvm, run:"
            notice "\t${tty_bold}nvm install 20${tty_normal}"
            notice "\t${tty_bold}nvm alias default 20${tty_normal}"
        else
            notice "nvm is not installed. Please follow the instructions at:"
            notice "\thttps://khanacademy.atlassian.net/wiki/spaces/ENG/pages/309625564/HOWTO+Upgrade+Node#Installing-from-scratch"
        fi
        read -p "Press enter to continue..."
    fi
}
